...
#define Result RTOTRR
	...
	if (RTAL & CK_ENSURE) {
		...
		tb1 = (((FUNCTION_CAST(EIF_TYPED_VALUE, (EIF_REFERENCE)) RTVF(10399, "is_open_read", Result))(Result)).it_b);
		if (tb1) {
			RTCK;
		} else {
			RTCF;
...
#define Result RTOTRR
	...
	RTOTP;
	RTHOOK(1);
	RTDBGAL(0, 0xF8000559, 0,0); /* Result */
	tr1 = RTLN(eif_new_type(1369, 0x01).id);
	(FUNCTION_CAST(void, (EIF_REFERENCE)) RTWC(11944, Dtype(tr1)))(tr1);
	RTNHOOK(1,1);
	Result = (EIF_REFERENCE) RTCCL(tr1);
	if (RTAL & CK_ENSURE) {
		RTHOOK(2);
		...
		tb1 = (((FUNCTION_CAST(EIF_TYPED_VALUE, (EIF_REFERENCE)) RTVF(7967, "is_open_write", Result))(Result)).it_b);
		if (tb1) {
			RTCK;
		} else {
			RTCF;
...
#define Result RTOTRR
	...
	(FUNCTION_CAST(void, (EIF_REFERENCE)) RTWC(11942, Dtype(tr1)))(tr1);
	RTNHOOK(1,1);
	Result = (EIF_REFERENCE) RTCCL(tr1);
	if (RTAL & CK_ENSURE) {
		RTHOOK(2);
